...
Portado del bot Workana original.
"""
import logging
import random
...
def _build_prompt(job: dict, profile: str, invitacion: str) -> str:
...
def generate_proposal(job: dict, profile: str = "") -> str:
    ...
    try:
        msg = client.messages.create(
            model=CLAUDE_MODEL,
            max_tokens=900,
            messages=[{"role": "user", "content": _build_prompt(job, profile, invitacion)}],
        )
